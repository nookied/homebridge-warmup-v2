...
**Current version:** **3.10.2** (UX tweak — `Service.Thermostat` is now added before `Service.TemperatureSensor` so Apple Home renders the thermostat tile to the left of the air sensor in the accessory detail view. Only affects newly-created accessories; existing cached ones keep their original order until reset. v3.10.2 is a republish of v3.10.1 with the GitHub URL fixed in `package.json` after the repo rename; v3.10.1 is tagged but never reached npm. Published 2026-05-06). Major v3 milestones: 3.0 GraphQL + per-room Off, 3.1 dynamic platform / Verified-eligible, 3.2 fakegato history, 3.3 sensor faults + runMode polish, 3.4 real heating signal + offline detection + override countdown, 3.5 Eve energy graphs + device firmware, 3.6 Vacation/Frost switches, 3.7 child lock, 3.8 validation polish, 3.9 review-pass correctness fixes, 3.9.1 HAP version hotfix, 3.10 feature toggles, 3.10.1 service-order tweak (failed to publish), 3.10.2 GitHub-URL fix + service-order tweak. Unreleased changes (if any) on `main` — check `git log v3.10.2..main`.
