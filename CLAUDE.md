...
**Current version:** **3.13.2** (attribution and doc-accuracy release; no functional change beyond one log level. Adds `NOTICE` and per-file provenance headers on `src/index.js` and `src/lib/warmup4ie.js`, now that leaving the GitHub fork network removed the only place GitHub itself stated the lineage; README credits what NorthernMan54 actually contributed, and adds simont77 and jondarrer, who were missing. Fixes three places — `config.schema.json`, the README config table, an `src/index.js` comment — that still told users `disableHistory` saves ~65 MB by avoiding `googleapis`, which 3.13.1 had already removed for everyone; **no figure is quoted now**, because none has been measured since, and `QA_TESTS.md` says to record one if anyone does. `loadFakeGatoHistory()` warns instead of whispering at debug when the bundled copy fails to load. 164 offline tests. Published 2026-08-30.) Previous: **3.13.1** (vendored `fakegato-history` with the Google Drive backend removed, so `googleapis` and its 93 transitive packages are gone: install 228 MB → 356 kB, one runtime dependency (`debug`), child-bridge RSS ~170 MB → 109.6 MB measured on a Pi 5. The copy is guarded by `test/unit/vendored-fakegato.test.js` — `eslint.config.mjs` ignores `src/vendor/` and `platform-state.test.js` mocks the module, so nothing else in CI would notice a broken copy, and `loadFakeGatoHistory()` failed silently by design — corrected in 3.13.2, it warns now. Homebridge Verified badge added to the README. 163 offline tests; live-tested. Published 2026-08-30.) Previous: **3.13.0** (ROADMAP M8 — the secondary temperature tile now shows the reading the Thermostat is *not* showing, named from the device's own `secondaryLabel`, so an air-mode device with a floor probe gets a "<room> Floor" tile. Backwards-compatible: with one probe `secondaryTemp` is null, the value falls back to `airTemp` and the label stays "Air". The `900` no-probe sentinel is filtered from probe fields but deliberately not from `currentTemp`, where 90 °C would be alarming but legitimate. `logSensorModes()` states each Thermostat's sensor mode at startup. Live-tested; 158 tests. Published 2026-08-28.) Major v3 milestones: 3.0 GraphQL + per-room Off, 3.1 dynamic platform / Verified-eligible, 3.2 fakegato history, 3.3 sensor faults + runMode polish, 3.4 real heating signal + offline detection + override countdown, 3.5 Eve energy graphs + device firmware, 3.6 Vacation/Frost switches, 3.7 child lock, 3.8 validation polish, 3.9 review-pass correctness fixes, 3.9.1 HAP version hotfix, 3.10 feature toggles, 3.10.1 service-order tweak (failed to publish), 3.10.2 GitHub-URL fix + service-order tweak (turned out to be a no-op visually), 3.10.3 link air sensor under thermostat (broke accessory rename), 3.10.4 unlink rollback, 3.11.0 disableAirSensor toggle, 3.12.0 maintenance + adversarial pass (Node range, login errors, null/NaN guards, write serialization, live room discovery, disableHistory), 3.12.1 reliability + docs audit, 3.13.0 labelled secondary reading (M8), 3.13.1 vendored fakegato + Verified badge + vendor guard test, 3.13.2 attribution + stale-rationale fixes. Unreleased changes (if any) on `main` — check `git log v3.13.2..main`.
...
│   │   └── vendored-fakegato.test.js         Vendored fakegato: factory loads, fs round-trip, no Drive
│   ├── integration/
...
| `test/unit/vendored-fakegato.test.js` | The only automated coverage `src/vendor/` has. Loads the factory as `src/index.js` does, drives the fs round-trip through `fakegato-storage.js`, and asserts the Drive backend stays gone in source and at runtime. **Do not delete when re-syncing the vendored copy** — eslint ignores that directory and the platform tests mock the module, so nothing else would catch a bad copy, and the runtime failure is silent. |
