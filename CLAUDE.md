...
**Current version:** **3.10.4** (regression rollback — v3.10.3's `addLinkedService(tempService)` caused iOS Home to refuse accessory renames with "Could not change settings". v3.10.4 removes the call and actively unlinks any link previously persisted to the cached accessory state, so users don't need another cache reset. Visually this lands back at v3.10.2 behaviour: TemperatureSensor renders as a sibling tile next to the Thermostat. The next attempt at the "thermostat-only" UX will be a `disableAirSensor` opt-out toggle. Published 2026-05-06). Major v3 milestones: 3.0 GraphQL + per-room Off, 3.1 dynamic platform / Verified-eligible, 3.2 fakegato history, 3.3 sensor faults + runMode polish, 3.4 real heating signal + offline detection + override countdown, 3.5 Eve energy graphs + device firmware, 3.6 Vacation/Frost switches, 3.7 child lock, 3.8 validation polish, 3.9 review-pass correctness fixes, 3.9.1 HAP version hotfix, 3.10 feature toggles, 3.10.1 service-order tweak (failed to publish), 3.10.2 GitHub-URL fix + service-order tweak (turned out to be a no-op visually), 3.10.3 link air sensor under thermostat (broke accessory rename), 3.10.4 unlink rollback. Unreleased changes (if any) on `main` — check `git log v3.10.4..main`.
